...
struct student_node *search_node_student(struct student_node *head,
struct student_data student_new, int mode)
{
	...
	case 1:
		{
			while (temp != NULL) {
				...
				cmp = strcmp(temp->student_new.name,
student_new.name);
				if (cmp == 0) {
					/* jump here*/
	...
	case 2:
		{
			while (temp != NULL) {
				...
				cmp = strcmp(temp->student_new.age,
student_new.age);
	...
	case 3:
		{
			while (temp != NULL) {
				...
				cmp = strcmp(temp->student_new.score,
student_new.score);
				if (cmp == 0) {
					/* jump here*/
					out_student_info(temp->student_new);
